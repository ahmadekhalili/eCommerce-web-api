...
class CartTestCase(TestCase):                             # note: ShopFilterItemCartTestCase use this class and run its methods, so in using cart methods like cart.add or cart.remove you should provide shopfilteritem_id argument
    ...
    def _create_item(self, **kwargs):
        return {'product_id': Product.objects.create(**kwargs).id, 'shopfilteritem_id': None}
    ...
    def test_cart_iter(self):
        ...
        item1, item2 = ShopFilterItem.objects.create(product=product2, stock=10, price=86945056), ShopFilterItem.objects.create(product=product2, stock=1, price=10000)
        cart = self._login(self.request, self.user)

        cart.add(product1.id, quantity=7)
        cart.add(item1.product.id, quantity=7, shopfilteritem_id=item1.id)
        cart.add(item2.product.id, quantity=1, shopfilteritem_id=item2.id)
        cart.add(product3.id, quantity=1)

        # test old_price price_changes ----------------------------------
        # change product2.price to make sure dont affect on its shopfilteritems prices (item1, item2)
        cart.cart_page = True
        product1.price, product2.price, item1.price = 11000, 10000, 86944056
        for item in [product1, product2, item1]: item.save()
        # loop over cart like "for item in cart"
        items = list(iter(cart))
        # product1 price increased 1000, item1 decreased 1000
        ...
        self.assertTrue(items[0]['old_price'] == cart.session[settings.CART_SESSION_ID][product1.id]['old_price'] == '11000')
        self.assertTrue(items[1]['old_price'] == cart.session[settings.CART_SESSION_ID][item1.product.id][item1.id]['old_price'] ==  '86944056')
        items = list(iter(cart))
        ...
        product1.price, item1.price = 12000, 86943056
        for item in [product1, product2, item1]: item.save()
        items = list(iter(cart))
        self.assertEqual(items[0]['price_changes'], Decimal(1000))
        self.assertEqual(items[1]['price_changes'], Decimal(-1000))
        ...
        product1.price, item1.price = 14000, 86941056
        for item in [product1, item1]: item.save()
        items = list(iter(cart))
        # product1 price increased 2000, item1 decreased 2000
        ...
        product1.price, item1.price = 15000, 86940056
        for item in [product1, product2, item1]: item.save()
        items = list(iter(cart))
        ...
        cart.cart_page = False
        items = list(iter(cart))
        # lach_quantity of product1 is 1 and lach_quantity of item1 is 2
        ...
        self.assertTrue(items[0]['quantity'] == cart.session[settings.CART_SESSION_ID][product1.id]['quantity'] == 7)
        self.assertTrue(items[1]['quantity'] == cart.session[settings.CART_SESSION_ID][item1.product.id][item1.id]['quantity'] == 7)
        cart.cart_page = True
        items = list(iter(cart))
        # when cart_page is True quantity should be stock of its item
        self.assertTrue(items[0]['quantity'] == cart.session[settings.CART_SESSION_ID][product1.id]['quantity'] == 6)
        self.assertTrue(items[1]['quantity'] == cart.session[settings.CART_SESSION_ID][item1.product.id][item1.id]['quantity'] == 5)
...
class ShopFilterItemCartTestCase(CartTestCase):               #repeat CartTestCase tests with create ShopFilterItem istead product
    ...
    def _create_item(self, **kwargs):
        ...
        return {'product_id': p.id, 'shopfilteritem_id': ShopFilterItem.objects.create(product=p, stock=kwargs['stock'], price=0).id}
