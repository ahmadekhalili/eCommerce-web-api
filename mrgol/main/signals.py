...
class FillCategoryfilters_brands:
    ...
    def category_brand_changed(current_product):            # this method used in product.save(). with this we can identify product.category is changed in last product saving or not.
        ...
        if current_category and current_category != pre_category:       # Add Phase, should come to this condition: 1- if we add category1 to product1.category for first time like: p1=Product.objects.create(name='p1', filter_attributes=1) p1.category=category1 p1.save() current_category=category1 pre_category=None (note in first product's creation like: p1=Product.objects.create(name='p1', filter_attributes=1, category=1) here filters_id==None and filling category done by 'filter_attributes_changed'  2- if we edit product1.category like: product.category = category2 product.save(), current_category=category2 and pre_category=category1    should not come to this condition: 1- if we delete product1.category=category1, here current_category=None and pre_category=category1  2- if we edit eny fields of product1  except .category, like edit product.name ... current_category==pre_category
        ...
        if pre_category and current_category.id != pre_category.id:     # Remove Phase, should came to this condition: 1- if we edit product1.category like: product.category = category2 product.save(), current_category=category2 and pre_category=category1  2- if we delete product1.category, here current_category=None and pre_category=category1
        ...
        else:          # but what should happen if we only edit brand without changing category? we should cover it too.
