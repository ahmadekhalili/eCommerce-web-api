...
class FillRootfilters_brands:
    ...
    def root_brand_changed(current_product):            # this method used in product.save(). with this we can identify product.root is changed in last product saving or not.
        ...
        if pre_root and current_root.id != pre_root.id:     # Remove Phase, should came to this condition: 1- if we edit product1.root like: product.root = root2 product.save(), current_root=root2 and pre_root=root1  2- if we delete product1.root=root1, here current_root=None and pre_root=root1
