...
class Cart(object):
    ...
    def add(self, product_id, quantity=1, shopfilteritem_id=None):             # in addingm product_id/shopfilteritem_id can be str or int hasn't difference, but in save they both save only str. example: Cart(request).cart[str(product1.id)]
        product_id, quantity, shopfilteritem_id = str(product_id), int(quantity), str(shopfilteritem_id) if shopfilteritem_id else shopfilteritem_id                 # product_id (or shopfilteritem_id dont different) shoud be str, because after saving data and keys in self.sassion in 'def save', keys will converte to str automatically (example: cart=Cart(request)  cart.add(1, 1, None)  again_cart=Cart(request)  again_cart.cart is like:  {'1': {'quantity': 1, ....}}   (doc says: "Use normal Python strings as dictionary keys on request.session. This is more of a convention than a hard-and-fast rule.")
        item = get_object_or_404(ShopFilterItem, id=shopfilteritem_id) if shopfilteritem_id else get_object_or_404(Product, id=product_id)                           # in getting products, ids could be str or int isn't difference
            ...
            if product:
                ...
                for shopfilteritem_id in loop:                                                                               # should loop one time if we have not shopfilteritem
                    if self.is_nested_dict(self.cart, id):
                        item, mutable_item = {**self.cart[id][shopfilteritem_id].copy(), 'product': product}, self.cart[id][shopfilteritem_id]
                    else:
                        item, mutable_item = {**self.cart[id].copy(), 'product': product}, self.cart[id]     # item = self.cart[key] is mutable so every change in item, affect self.cart and self.session and self.request
                    item['shopfilteritem'] = ids_shopfilteritems.get(shopfilteritem_id)
                    item['price'] = item['shopfilteritem'].price if item['shopfilteritem'] else product.price        # django_rest framework convert decimal to str so we convert to str not int!
                    ...
                    item['lach_quantity'] = item['quantity']-item['shopfilteritem'].stock if item['shopfilteritem'] and item['quantity']>item['shopfilteritem'].stock else item['quantity']-product.stock if not item['shopfilteritem'] and item['quantity']>product.stock else 0       # lach_quantity fill when item.stock decreased by admin
